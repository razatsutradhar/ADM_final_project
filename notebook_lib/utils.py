...
def view_output(archive_path: str, title: str):
    ...
    # all_plot = all.plot(ax=ax, column=column_name, legend=True, cmap="viridis", )
    all_plot = all.plot(ax=ax, column=column_name, legend=True )
    # cbar=all_plot.colorbar(column_name, size='3%',extend="max",location="bottom")
    # plt.axis("off")
    # plt.title(title)
    # plt.colorbar(all_plot.get_children()[0], ax=ax, orientation="horizontal")
    # plt.clim(0, 1)
...
def view_raster_heatmap(
    ...
    plt.imshow(
        ...
        np.ma.MaskedArray(data=gt_out_cluster_m_new, mask=~mask1), 
    )
    plt.legend()
    plt.axis("off")
